...
# from image_utils import normalize_scan
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fp_model',
                        action = 'store',
                        required=False,
                        help='Model for fp reduction')
    ...
    if opt.fp_model is not None:
        use_fp = True
        fp_model = opt.fp_model
        print("using FP Reduction, loading Model")
        current_path = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(current_path, 'output', fp_model)
        fp_learner = load_learner(model_path)
        tp_threshold = 0.05 

    else: print (" NO FP reduction")
    ...
    output_csv = output_file +"fp"+ str(tp_threshold)+"_"+ str(depth_d)+'slices'+str(start_pred)+'s_sc'+str(min_score)+'_iou'+ str(min_iou)+'.csv'
        ...
        #     print(pred_boxes_vol)
            ...
            for box,score,slice_i in listScores:
                if (n_boxes < max_n_boxes):
                    ...
                    image_2d = normalize_scan(image_2d)                                   
                    if debug:
                        cv2.rectangle(image_2d,(int(box[0]),int(box[1])),(int(box[2]), int(box[3])),(0,0,0),5)
                        cv2.imwrite(debug_dir+"/"+client+"_"+episode+"_"+view+"_"+str(slice_i)+"_"+str(score)+".png", image_2d)
                        # todo: retrain without the 5 pixels border... and move image_crop before rectangle.
                    image_crop = image_2d[int(box[1]): int(box[3]), int(box[0]):int(box[2])]                    
                    # FP reduction 
                    prob_tp = fp_learner.predict(image_crop)[2][1].numpy()
                    # print(fp_learner.predict(image_crop))
                    print ("FP",prob_tp)
                    if prob_tp > tp_threshold:     # cound as a prediction
                        # score = (score + prob_tp) / 2
                        tp_boxes_vol.append(box)
                        tp_scores_vol.append(score)
                        tp_slices_vol.append(slice_i)
                        n_boxes +=1
                        if debug:
                            cv2.imwrite(debug_dir+"/rois_tp/"+client+"_"+episode+"_"+view+"_"+str(slice_i)+"_"+str(score)+".png", image_crop)
                            # todo: retrain without the 5 pixels border... and move image_crop before rectangle.
                            print (box, score,slice_i)
            # write_csv (final_boxes_vol, final_scores_vol, final_slices_vol, client, episode, view, dims[0],final_tp
            write_csv (tp_boxes_vol, tp_scores_vol, tp_slices_vol, client, episode, view, dims[0])
            #input("Press Enter to continue...")
